#!/bin/sh
echo
echo "[RFSH] Job f6f89824-5d10-11ee-8c9d-f297ad799bdb"
echo
RUN_DIR="$(mktemp -d)"
...
# install runflow
runflow_sh=runflow
if ! [ -x "$(command -v runflow)" ] || ! [[ "$(runflow version)" =~ "v0.1.9" ]]; then
  echo '[RFSH] Downloading runflow...'
  echo
  pushd ${RUN_DIR} > /dev/null 2>&1 && \
    n=0 && \
      until [ "$n" -ge 3 ]; do
      curl -C - --retry 3 -sL https://raw.githubusercontent.com/docsion/rfsh/main/i.sh | VERSION=v0.1.9 sh && break
      n=$((n+1))
      sleep 15
      done && \
    popd > /dev/null 2>&1
  ...
  echo [RFSH] Found runflow v0.1.9 at $(which runflow)
  echo
fi
